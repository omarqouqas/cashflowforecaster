...
function getFrequencyBadge(frequency: string | null | undefined) {
  ...
  if (freq === 'quarterly') {
    return {
      label: 'Quarterly',
      className: 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-500/30'
    }
  }
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="text-base font-semibold text-zinc-900 dark:text-zinc-100 truncate">{income.name}</p>
                {isInvoiceLinked && (
                  <span
                    className={[
                      'inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold whitespace-nowrap',
                      isConfirmed
                        ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30'
                        : isPending
