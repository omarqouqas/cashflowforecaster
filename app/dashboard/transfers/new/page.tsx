...
export default function NewTransferPage() {
  ...
  // Get prefill params from URL (for CC payment shortcut)
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Set prefilled values after accounts load
      if (prefillFrom && accountsResult.data?.find(a => a.id === prefillFrom)) {
        setValue('from_account_id', prefillFrom);
      }
      if (prefillTo && accountsResult.data?.find(a => a.id === prefillTo)) {
        setValue('to_account_id', prefillTo);
      }
      if (prefillAmount) {
    ...
    fetchData();
  }, [prefillFrom, prefillTo, prefillAmount, setValue]);
  ...
  const onSubmit = async (data: TransferFormData) => {
    ...
    showSuccess('Transfer scheduled');
    router.refresh();
  ...
  if (dataLoading) {
    return (
      <div className="max-w-lg mx-auto">
        ...
        <h1 className="text-2xl font-semibold text-zinc-100 mb-6">New Transfer</h1>
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="border border-zinc-700/50 bg-zinc-800/50 backdrop-blur-sm rounded-lg p-6 shadow-xl">
          <div className="text-center py-8">
            <p className="text-zinc-400 mb-4">You need at least 2 accounts to create a transfer.</p>
            <Link
              href="/dashboard/accounts/new"
              className="inline-flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white font-medium rounded-md px-4 py-2 transition-colors"
            >
              Add Account
            </Link>
          </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <h1 className="text-2xl font-semibold text-zinc-100 mb-2">
        {isCCPayment ? 'Pay Credit Card' : 'New Transfer'}
      </h1>
      <p className="text-zinc-400 text-sm mb-6">
        {isCCPayment
          ? 'Schedule a payment from your bank account to your credit card'
      ...
      <div className="border border-zinc-700/50 bg-zinc-800/50 backdrop-blur-sm rounded-lg p-6 shadow-xl">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t border-zinc-700/50">
            ...
            >
              {isLoading ? 'Creating...' : isCCPayment ? 'Schedule Payment' : 'Create Transfer'}
            </button>
          </div>
        </form>
      </div>
